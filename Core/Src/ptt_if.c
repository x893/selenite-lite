...
static uint32_t HEX_to_BCD(uint32_t input)
{
	int8_t   i;
	uint32_t output = 0U;
...
static uint32_t BCD_to_CAT(uint32_t input)
{
	int8_t   i;
	uint32_t output = 0U;
...
static void ptt_set_bpf(uint32_t tune_new)
{
	uint8_t retval = 0x06;
...
static void ptt_set_tx(void)
{
	...
	if (!trx.is_tx)
	{
		...
		ptt_set_bpf(trx.vfo ? trx.vfob : trx.vfoa);
	}
}
...
void VFO_Set_Tune(uint32_t tune_new)
{
	...
	case 0:
		...
		{
			trx.vfoa = tune_new;
			trx.vfoa_bcd = HEX_to_BCD(trx.vfoa / 10);
	...
	case 1:
		...
		{
			trx.vfob = tune_new;
			trx.vfob_bcd = HEX_to_BCD(trx.vfob / 10);
			trx.vfob_bcd = BCD_to_CAT(trx.vfob_bcd);
		}
		break;
	}

	Si5351a_Set_Freq(tune_new);
...
{
	return trx.vfo ? trx.vfob : trx.vfoa;
}
...
{
	return trx.vfo ? trx.vfob_bcd : trx.vfoa_bcd;
}
